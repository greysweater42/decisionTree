...
#' @param dict a data.frame of optional names for nodes
...
plot.DecisionTreeObject <- function(DecisionTreeObject, dict, green=1, left=1) {
    ...
    if (green == 1) {
        color <- paste0("#", as.hexmode(round(255 - cols)), as.hexmode(round(cols)), "00")
    } else {
        color <- paste0("#", as.hexmode(round(cols)), as.hexmode(round(255 - cols)), "00")
    }
    ...
    if (missing(dict)) {
        ...
        rect(d$x - widths, d$y - 0.15 * (nLevel - 1) / 3,  
             d$x + widths, d$y + 0.15 * (nLevel - 1) / 3, col=color, border="gray")
        text(d$x, d$y, d$name)
    } else {
        ...
        rect(dd$x - widths, dd$y - 0.15 * (nLevel - 1) / 3,
             dd$x + widths, dd$y + 0.15 * (nLevel - 1) / 3, col=color, border="gray")
...
# ---- example
# library(data.table)
# setwd('/home/tomek/Desktop/przetarg')
# dRaw <- fread('./data.csv')
# d <- cbind(dRaw$wynik, dRaw$inLiczbaSpotkanNewBiz, dRaw[, substr(colnames(dRaw), 1, 2) == "ex", with=F])
# colnames(d)[1] <- "wynik"
# colnames(d)[2] <- "inLiczbaSpotkanNewBiz"
# d <- d[wynik %in% c("wygrany", "przegrany")]
# d <- d[exZakresPrzetargu %in% c("online", "offline", "wszystko")]
# testData <- d[!is.na(exOkresOdPoprzPrzetargu)]
# testData$exBudzet <- NULL
# testData <- as.data.frame(testData, stringsAsFactors=F)
# tr <- decisionTree(testData, eta=23, purity=0.85, minsplit=10)
# dict <- data.frame(vName=c("exOkresOdPoprzPrzetargu",
#                            "exGotowoscKlientaDoZmiany",
#                            "exRelacjaZDecyzyjnymi",
#                            "exZakresPrzetargu",
#                            "exNaszeDoswiadczenieWBranzy"),
#                    toPlot=c("Okres od poprzedniego przetargu",
#                             "Gotowość klienta do zmiany",
#                             "Relacja z osobami decyzyjnymi",
#                             "Zakres przetargu",
#                             "Nasze doświadczenie w branży"),
#                    stringsAsFactors=F)
# 
# plot(tr, dict)
